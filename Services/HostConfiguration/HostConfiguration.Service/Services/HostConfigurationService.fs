...
type HostConfigurationService(logger: ILogger<HostConfigurationService>) =
    ...
    override this.SetHosts(request: HostChangeRequest, context: ServerCallContext) : Task<HostChangeResponse> =
        ...
        let tlsClusterIssuer =
            if tlsClusterIssuerEnv = null then
                None
            else
                Some tlsClusterIssuerEnv
        ...
        async {
            ...
            let! currentIngressRule =
                async {
                    try
                        let! rule =
                            cluster.ReadNamespacedIngressAsync(ingressName, targetNamespace)
                            |> Async.AwaitTask
            ...
            | Some issuer ->
                newIngressRule.Metadata.Annotations.Add("cluster-issuer.io/ingress", issuer)

                newIngressRule.Spec.Tls <-
                    [| new V1IngressTLS(secretName = $"tls-secret-ingress-{request.AccountID}", hosts = request.Hosts) |]
            | None -> ()

            match currentIngressRule with
            | None ->
                do!
                    cluster.CreateNamespacedIngressAsync(newIngressRule, targetNamespace)
                    |> Async.AwaitTask
                    |> Async.Ignore
            | Some _ ->
                do!
                    cluster.ReplaceNamespacedIngressAsync(newIngressRule, ingressName, targetNamespace)
                    |> Async.AwaitTask
                    |> Async.Ignore
    ...
    override this.RemoveHost(request: HostRemoveRequest, context: ServerCallContext) : Task<HostChangeResponse> =
        ...
        async {
            ...
            do!
                cluster.DeleteNamespacedIngressAsync(ingressName, targetNamespace)
                |> Async.AwaitTask
                |> Async.Ignore
